...
<div>
     <!-- HEADER -->
    {{-- <x-mary-header title="Welcome" separator progress-indicator>
    ...
    {{-- <x-mary-card>
        <x-mary-table :headers="$headers" :rows="$users" :sort-by="$sortBy">
            @scope('actions', $user)
            <x-mary-button icon="o-trash" wire:click="delete({{ $user['id'] }})" spinner class="text-red-500 btn-ghost btn-sm" />
            @endscope
        </x-mary-table>
    </x-mary-card> --}}
